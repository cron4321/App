  ...
  db.query(
    ...
    (err, results) => {
      ...
      if (results.length > 0) {
        ...
        const existingUsername = results.some(
...
app.get("/getInitialMessages", (req, res) => {
  connection.query("SELECT * FROM messages", (err, results) => {
    if (!err) {
      ...
      res
        .status(500)
        .json({ error: "초기 채팅 내용을 가져오는 중 오류가 발생했습니다." });
    }
  });
});
...
app.post("/select-school", requireLogin, (req, res) => {
  ...
  const insertOrUpdateSchool = (user, schoolName) => {
    const sql =
      "INSERT INTO schools (user_id, school_name) VALUES (?, ?) ON DUPLICATE KEY UPDATE school_name = ?";
