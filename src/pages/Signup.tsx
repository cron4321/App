...
function Signup() {
  ...
  const sendVerificationCode = async () => {
    try {
      const response = await axios.post('http://172.31.39.85:3002/send-verification-email', { email: user.email });
  ...
  const checkDuplicate = async () => {
    try {
      const response = await axios.post('http://172.31.39.85:3002/check-duplicate', {
        email: user.email,
  ...
  const handleSignUp = async () => {
    if (isEmailValid && isPasswordValid && isPasswordsMatching && user.email && user.password) {
      try {
        const response = await axios.post('http://172.31.39.85:3002/signup', user);
